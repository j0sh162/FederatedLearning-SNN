...
def load_dataset(name,path,num_partitions: int,batch_size: int,val_ratio: float=0.1):
    ...
    trainsets = random_split(trainSet, partition_length, torch.Generator().manual_seed(42)) #For repetition
    ...
    for partition in trainsets:
        ...
        for_training, for_validation = random_split(partition,[train_length,validation_length], generator=torch.Generator().manual_seed(42))
        trainingList.append(DataLoader(for_training, batch_size=batch_size, shuffle=True, num_workers=2))
        validationList.append(DataLoader(for_validation, batch_size=batch_size, shuffle=False, num_workers=2))
