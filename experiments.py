...
from SNN_Models import SNN_utils,Spide,EventProp

lr = 0.01 # TODO CHANGE THIS DEPENDING ON YOUR MODEL
device = (
            torch.device("cuda") if torch.cuda.is_available() else torch.device("cpu")
        )
# Agg FedAvg FedOpt FedProx
# Models SPIDE SG EventProp BioGrad
# Dataset NMNIST
# ROUNDS 10 EPOCHS 10
# Standalone 100 EPOCHS
# IDD non IDD
# 5 10 20 clients each 
# Need to ask natalia about the different aggregation stratergys 
# 

def standalone_train(model_cfg,model,trainloader,optim,epochs,device):
    if model_cfg._target_ == "SNN_Models.SNN.Net":
            SNN_utils.train(
                model, trainloader, optim, epochs, device
            )
    elif model_cfg._target_ == "SNN_Models.EventProp.SNN":
            EventProp.train(
                model, optim, trainloader, optim, device,logging=True
            )
    elif model_cfg._target_ == "SNN_Models.Spide.SNNSPIDEConvNet":
            Spide.train(
                model, trainloader, device, epochs, optim,logging=True
    )
...
def main(cfg: DictConfig):
    ...
    # standalone_train(cfg.model,trainLoaders[0],optim,100,device)
    ...
    for num_clients in [5,10,20]:
        ...
        cfg.fl.num_sampled_clients = num_clients
        cfg.client.num_clients_per_round_fit = num_clients
        cfg.client.num_clients_per_round_evaluate = num_clients
        cfg.strategy.min_fit_clients = num_clients
        cfg.strategy.min_evaluate_clients = num_clients
        cfg.strategy.min_available_clients = num_clients
        cfg.strategy.on_fit_config_fn.config = cfg.client
    ...
    # 6. Save results
        ...
        writer = SummaryWriter(log_dir=f"runs/federated_server/network:{cfg.model._target_}num_clients:{num_clients}seed:{seed}")
