...
class BioGradNetworkWithSleep(nn.Module):
    ...
    def train_online(self, spike_data, label_one_hot, soft_error_step):
        ...
        """
        # Input assumed shape: [time, batch, channels, width, height]
        spike_data = spike_data.permute(1, 2, 3, 4, 0).float()  # → [batch, channels, width, height, time]

        batch_size = spike_data.shape[0]
        channels = spike_data.shape[1]
        width = spike_data.shape[2]
    ...
    def test(self, spike_data):
        ...
        """
        print("Original spike_data.shape:", spike_data.shape)

        # Correct permutation: [time, batch, channels, width, height] → [batch, channels, width, height, time]
        # spike_data: [time, batch, channels, width, height]
        spike_data = spike_data.permute(1, 2, 3, 4, 0).float()  # → [batch, channels, width, height, time]
