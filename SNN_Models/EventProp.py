...
def train(model, optimizer, loader, epochs, device, logging=False):
    ...
    writer = SummaryWriter(f"runs/EventProp_centralized/seed_{0}") if logging else None
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (input, target) in enumerate(iter(loader)):
            ...
            if alpha != 0:
                ...
                loss += (
                    alpha
                    * (torch.exp(target_first_spike_times / (beta * tau_s)) - 1).mean()
                )
            ...
            if batch_idx % 10 == 0:
                print(
                    "\tBatch {:03d}/{:03d}: \tAcc {:.2f}  Loss {:.3f}".format(
                        batch_idx,
                        len(loader),
                        100 * total_correct / total_samples,
                        total_loss / total_samples,
...
def test(model, loader, device):
    ...
    with torch.no_grad():
        for batch_idx, (data, target) in enumerate(loader):
            ...
            if batch_idx % 10 == 0:
                print(
                    "\tBatch {:03d}/{:03d}: \tAcc {:.2f}".format(
                        batch_idx, len(loader), 100 * total_correct / total_samples
                    )
                )
...
if __name__ == "__main__":
    ...
    transform = tonic.transforms.Compose(
        [
            ...
            tonic.transforms.ToFrame(
                sensor_size=tonic.datasets.NMNIST.sensor_size, n_time_bins=args.T
            ),
        ]
    )
    ...
    model = SNN(2312, 10, args.T, args.dt, args.tau_m, args.tau_s, args.xi, 0.01, 2).to(
        device
    )
    ...
    train(model, optimizer, train_loader, args.epochs, device, logging=True)
